...
from src.client.retrieval_client_visual import RemoteRetrievalClientVisual
from src.utils.utils import get_timestamp, load_yaml, save_json
...
# Load configuration
config = load_yaml(args.client_config_path)
logger.info(f"Loaded config from {args.client_config_path}")
    ...
    with gr.Tab("Image Search"):
        ...
        with gr.Row():
            image_gallery = gr.Gallery(
                ...
                columns=5,
                rows=1,
                visible=config.get("SHOW_IMAGE_GALLERY", True),
                show_label=True
            )
            ...
            for i in range(image_top_k.value):
                with gr.Column():
                    annotator = image_annotator(
                        ...
                        label=f"Result {i + 1}",
                        visible=config.get("SHOW_ANNOTATORS", True),
                        sources=[],
                    )
        ...
        with gr.Row():
            with gr.Column():
                relevant_captions = gr.Textbox(
                    label="Relevant details about the image",
                    placeholder="Enter some relevant details about the image here..."
                )
                irrelevant_captions = gr.Textbox(
                    label="Irrelevant details about the image",
                    placeholder="Enter some irrelevant details about the image here..."
                )
